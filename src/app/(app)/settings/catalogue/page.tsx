...
export default async function CataloguePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <Link href="/settings" className="inline-flex items-center gap-2 text-sm text-[#4b5563]">
        <ArrowLeft className="size-4" /> Back to settings
      </Link>
      <p className="mt-6 text-sm font-semibold uppercase tracking-[0.16em] text-[#011478]">Reusable tour content</p>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-3">
        <section className="rounded-xl border bg-white p-5">
          <div className="flex items-center gap-3">
            <MapPin className="size-5 text-[#011478]" />
            <div><h2 className="font-semibold">Destinations</h2><p className="text-xs text-[#6b7280]">{data.destinations.length} records</p></div>
          </div>
          <div className="mt-4 max-h-56 divide-y overflow-y-auto">
            {data.destinations.map((item) => (
              <div key={item.id} className="py-3">
                <p className="text-sm font-medium">{item.name}</p>
                <p className="mt-1 text-xs text-[#6b7280]">{item.country} - {item._count.activities} activities - {item._count.accommodations} stays</p>
              </div>
            ))}
          </div>
          <details className="mt-4 border-t pt-4">
            <summary className="cursor-pointer text-sm font-semibold text-[#011478]">Add destination</summary>
            <form action={createDestinationAction} className="mt-4 space-y-3">
              ...
              <button className="h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white">Save destination</button>
            </form>
          </details>
        </section>
        ...
        <section className="rounded-xl border bg-white p-5">
          <div className="flex items-center gap-3">
            <Ticket className="size-5 text-[#011478]" />
            <div><h2 className="font-semibold">Activities</h2><p className="text-xs text-[#6b7280]">{data.activities.length} records</p></div>
          </div>
          <div className="mt-4 max-h-56 divide-y overflow-y-auto">
            {data.activities.map((item) => (
              <div key={item.id} className="py-3">
                <p className="text-sm font-medium">{item.name}</p>
                <p className="mt-1 text-xs text-[#6b7280]">{item.destination.name} - {item.category}</p>
                {item.rates[0] ? <p className="mt-1 text-xs font-semibold text-[#011478]">From {formatMoney(item.rates[0].amount.toString(), item.rates[0].currencyCode)}</p> : null}
              </div>
            ))}
          </div>
          <details className="mt-4 border-t pt-4">
            <summary className="cursor-pointer text-sm font-semibold text-[#011478]">Add activity</summary>
            <form action={createActivityAction} className="mt-4 space-y-3">
              ...
              <button className="h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white">Save activity</button>
            </form>
          </details>
        </section>
        ...
        <section className="rounded-xl border bg-white p-5">
          <div className="flex items-center gap-3">
            <BedDouble className="size-5 text-[#011478]" />
            <div><h2 className="font-semibold">Accommodation</h2><p className="text-xs text-[#6b7280]">{data.accommodations.length} records</p></div>
          </div>
          <div className="mt-4 max-h-56 divide-y overflow-y-auto">
            {data.accommodations.map((item) => (
              <div key={item.id} className="py-3">
                <p className="text-sm font-medium">{item.name}</p>
                <p className="mt-1 text-xs text-[#6b7280]">{item.destination.name} - {item.type} - {item.roomTypes.length} room types</p>
                {item.rates[0] ? <p className="mt-1 text-xs font-semibold text-[#011478]">From {formatMoney(item.rates[0].amount.toString(), item.rates[0].currencyCode)}</p> : null}
              </div>
            ))}
          </div>
          <details className="mt-4 border-t pt-4">
            <summary className="cursor-pointer text-sm font-semibold text-[#011478]">Add accommodation</summary>
            <form action={createAccommodationAction} className="mt-4 space-y-3">
              ...
              <button className="h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white">Save accommodation</button>
            </form>
          </details>
        </section>
      </div>
      ...
      <section className="mt-6 rounded-xl border bg-white p-5">
        <div className="flex items-center gap-3"><Compass className="size-5 text-[#011478]" /><div><h2 className="font-semibold">Rates and room types</h2><p className="text-xs text-[#6b7280]">New effective dates create historical rate records; existing rates are never overwritten.</p></div></div>
        <div className="mt-5 grid gap-5 lg:grid-cols-3">
          <details className="rounded-lg border p-4">
            ...
            <form action={addActivityRateAction} className="mt-4 space-y-3">
              ...
              <button className="h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white">Save rate</button>
            </form>
          </details>
          <details className="rounded-lg border p-4">
            ...
            <form action={addRoomTypeAction} className="mt-4 space-y-3">
              ...
              <button className="h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white">Save room type</button>
            </form>
          </details>
          <details className="rounded-lg border p-4">
            ...
            <form action={addAccommodationRateAction} className="mt-4 space-y-3">
              ...
              <p className="rounded-lg bg-[#eff3ff] px-3 py-2 text-[11px] text-[#4b5563]">Occupancy and accommodation are taken automatically from the selected room type.</p>
