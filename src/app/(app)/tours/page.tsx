  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#176b55]">Central tour records</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">Tours</h1>
          <p className="mt-3 text-sm text-[#68736e]">Planning, pricing, booking, operations, payments, and profitability stay connected here.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href="/packages" className="flex h-11 items-center gap-2 rounded-xl border bg-white px-4 text-sm font-semibold"><PackageOpen className="size-4" /> Standard packages</Link>
          <Link href="/tours/new" className="flex h-11 items-center gap-2 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white"><Plus className="size-4" /> Create new tour</Link>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-sm">
            ...
            <tbody className="divide-y">
              {tours.map((tour) => (
                <tr key={tour.id} className="hover:bg-[#fafaf7]">
                  <td className="px-6 py-4">
                    <p className="font-semibold">{tour.name}</p>
                    <p className="mt-1 text-xs text-[#7b8580]">{tour.reference}</p>
                  </td>
                  <td className="px-6 py-4">{tour.customer.fullName}</td>
                  <td className="px-6 py-4 text-[#68736e]">
                  ...
                  <td className="px-6 py-4 text-[#68736e]">{tour.owner.fullName}</td>
                  <td className="px-6 py-4">
                    <Link href={`/tours/${tour.id}`}><ArrowRight className="size-4 text-[#176b55]" /></Link>
                  </td>
                </tr>
              ))}
              {!tours.length ? (
                <tr>
                  <td colSpan={8} className="px-6 py-16 text-center">
                    <MapPinned className="mx-auto size-6 text-[#a0a8a4]" />
                    <p className="mt-3 font-medium">No tours yet</p>
                    <p className="mt-1 text-xs text-[#7b8580]">Create one directly, from an enquiry, or from a standard package.</p>
                    <Link href="/tours/new" className="mt-4 inline-flex h-10 items-center rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white">Create first tour</Link>
                  </td>
                </tr>
              ) : null}
            </tbody>
