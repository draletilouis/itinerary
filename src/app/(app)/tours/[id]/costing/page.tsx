...
export default async function TourCostingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <p className="mt-6 text-sm font-semibold uppercase tracking-[0.16em] text-[#011478]">Multi-currency worksheet</p>
      <h1 className="mt-2 text-3xl font-semibold tracking-tight">{tour.name} costing</h1>
      <p className="mt-3 text-sm text-[#4b5563]">Original supplier currency is preserved; every item records the exact conversion rate used.</p>

      <TourWorkspaceNav tourId={tour.id} active="costing" itineraryId={tour.itineraries[0]?.id} bookingId={tour.booking?.id} />

      <section className="mt-7 grid gap-4 sm:grid-cols-3">
        <article className="rounded-xl border bg-white p-5"><Coins className="size-5 text-[#011478]" /><p className="mt-4 text-xs text-[#6b7280]">Internal cost</p><p className="mt-2 text-2xl font-semibold">{formatMoney(tour.estimatedInternalCost.toString(), tour.costingCurrencyCode)}</p></article>
        <article className="rounded-xl border bg-white p-5"><Calculator className="size-5 text-[#011478]" /><p className="mt-4 text-xs text-[#6b7280]">Customer price</p><p className="mt-2 text-2xl font-semibold">{formatMoney(tour.sellingPrice.toString(), tour.quotationCurrencyCode)}</p></article>
        <article className="rounded-xl border bg-white p-5"><TrendingUp className="size-5 text-[#011478]" /><p className="mt-4 text-xs text-[#6b7280]">Estimated margin</p><p className="mt-2 text-2xl font-semibold">{tour.estimatedMargin.toFixed(2)}%</p></article>
      </section>

      <section className="mt-6 overflow-hidden rounded-xl border bg-white">
        <div className="border-b px-5 py-4"><h2 className="font-semibold">Cost items</h2><p className="mt-1 text-xs text-[#6b7280]">{tour.costItems.length} recorded items</p></div>
        <div className="border-b bg-[#f9fafb] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold">Import from itinerary</h3>
              <p className="mt-1 text-xs text-[#4b5563]">Review matched supplier rates, then import the selected lines. Unmatched items remain available for manual costing.</p>
            </div>
            ...
            <form action={importItineraryCostsAction} className="mt-4">
              ...
              <div className="space-y-2">
                {data.itineraryCostReview.suggestions.map((item) => (
                  <label key={item.itineraryItemId} className="flex items-start gap-3 rounded-lg border bg-white p-3 text-sm">
                    <input
                      className="mt-1 size-4 accent-[#011478]"
                      type="checkbox"
                      name="itemId"
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full bg-[#f3f4f6] px-2 py-0.5 text-[10px] uppercase text-[#4b5563]">{item.type.toLowerCase()}</span>
                      </span>
                      {item.status === "READY" ? (
              ...
              <button
                ...
                className="mt-4 h-10 rounded-lg bg-[#011478] px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
              >
                Import selected costs
              </button>
            </form>
          ) : (
            <p className="mt-4 rounded-lg border border-dashed bg-white p-4 text-sm text-[#4b5563]">No itinerary items are available in the current itinerary version.</p>
          )}
          ...
          <table className="w-full min-w-[1180px] text-left text-sm">
            ...
            <tbody className="divide-y">
              {tour.costItems.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td className="px-5 py-4">
                      <span className="rounded-full bg-[#eef3f1] px-2 py-1 text-[10px] font-semibold uppercase text-[#011478]">{display.basisLabel}</span>
                      <p className="mt-2 whitespace-nowrap text-xs text-[#4b5563]">{display.formula}</p>
                    </td>
                    <td className="px-5 py-4 font-medium">{formatMoney(item.originalTotal.toString(), item.originalCurrencyCode)}</td>
                    <td className="px-5 py-4">
                      <p className={display.sameCurrency ? "font-medium text-[#4b5563]" : "font-medium"}>{display.conversionLabel}</p>
                      <p className="mt-1 text-[11px] text-[#9ca3af]">{display.conversionDetail} · {item.exchangeRateDate.toLocaleDateString("en-UG")}</p>
                    </td>
                    <td className="px-5 py-4 text-base font-semibold">{formatMoney(item.convertedTotal.toString(), item.convertedCurrencyCode)}</td>
                    <td className="px-5 py-4 text-[#4b5563]">{item.supplier?.name ?? "Direct / internal"}</td>
                    <td className="px-5 py-4">
                      <details>
              ...
              {!tour.costItems.length ? <tr><td colSpan={7} className="px-5 py-10 text-center text-[#6b7280]">No cost items yet.</td></tr> : null}
            </tbody>
          </table>
        </div>
        <details className="border-t bg-[#f9fafb]">
          <summary className="cursor-pointer px-5 py-4 text-sm font-semibold text-[#011478]">Add cost item</summary>
          <TourCostForm
            tourId={tour.id}
      ...
      <section className="mt-6 rounded-xl border bg-white p-5">
        <h2 className="font-semibold">Tour pricing</h2><p className="mt-1 text-xs text-[#6b7280]">Each save creates an immutable pricing revision. A minimum margin applies only to this tour.</p>
        <TourPricingForm
          tourId={tour.id}
        ...
        />
        {latest ? <div className="mt-6 grid gap-3 rounded-lg bg-[#f9fafb] p-4 sm:grid-cols-4"><div><p className="text-xs text-[#6b7280]">Revision</p><p className="mt-1 font-semibold">#{latest.revision}</p></div><div><p className="text-xs text-[#6b7280]">Selling price</p><p className="mt-1 font-semibold">{formatMoney(latest.sellingPrice.toString(), latest.currencyCode)}</p></div><div><p className="text-xs text-[#6b7280]">Profit</p><p className="mt-1 font-semibold">{formatMoney(latest.estimatedProfit.toString(), latest.currencyCode)}</p></div><div><p className="text-xs text-[#6b7280]">Margin</p><p className="mt-1 font-semibold">{latest.estimatedMargin.toFixed(2)}%</p></div></div> : null}
      </section>
    </div>
  );
}
