    ...
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mt-6 rounded-xl border bg-white p-5">
        ...
        <form action={generateQuotationAction} className="mt-5 grid gap-4 sm:grid-cols-2"><input type="hidden" name="tourId" value={tour.id} />
          ...
          {pricing && (pricing.sellingPrice.lessThanOrEqualTo(pricing.internalCost) || pricing.estimatedMargin.lessThanOrEqualTo(0)) ? <label className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm font-medium text-amber-900 sm:col-span-2">Zero-profit override reason<textarea className="mt-2 min-h-20 w-full rounded-lg border bg-white p-3 text-sm" name="zeroMarginReason" required minLength={10} placeholder="Explain why this quotation may be issued at or below cost." /></label> : <input type="hidden" name="zeroMarginReason" value="" />}
