...
export default async function NotificationsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#011478]">Attention centre</p>
      <h1 className="mt-2 text-3xl font-semibold tracking-tight">Notifications</h1>
      <p className="mt-2 text-sm leading-6 text-[#4b5563]">Live, database-derived alerts for overdue work, financial exposure, expiring records, and serious incidents.</p>
      <div className="mt-7 rounded-xl border bg-white p-5"><div className="flex items-center gap-3"><BellRing className="size-5 text-[#011478]" /><p className="font-semibold">{total} item{total === 1 ? "" : "s"} need attention</p></div></div>

      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="space-y-3"><h2 className="flex items-center gap-2 font-semibold"><CalendarClock className="size-4 text-[#011478]" /> Overdue work</h2>
          {followUps.map((entry) => <Link key={entry.id} href={`/enquiries/${entry.enquiry.id}`} className="block rounded-xl border bg-white p-4"><p className="text-sm font-semibold">Follow up · {entry.enquiry.reference}</p><p className="mt-1 text-xs text-[#6b7280]">{entry.enquiry.customer.fullName} · due {entry.scheduledFor.toLocaleString("en-UG")}</p></Link>)}
          {tasks.map((entry) => <Link key={entry.id} href="/operations" className="block rounded-xl border bg-white p-4"><p className="text-sm font-semibold">Operational task · {entry.title}</p><p className="mt-1 text-xs text-[#6b7280]">{entry.tour.reference} · {entry.tour.name} · due {entry.dueDate?.toLocaleDateString("en-UG")}</p></Link>)}
        </div>
        ...
        <div className="space-y-3"><h2 className="flex items-center gap-2 font-semibold"><AlertTriangle className="size-4 text-red-700" /> Serious open incidents</h2>
          {incidents.map((incident) => <Link key={incident.id} href="/operations" className="block rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm font-semibold">{incident.reference} · {incident.title}</p><p className="mt-1 text-xs text-red-700">{incident.severity} · {incident.tour.reference} · {incident.tour.name}</p></Link>)}
        </div>
      </section>
      {!total ? <p className="mt-8 rounded-xl border bg-white p-8 text-center text-sm text-[#6b7280]">Nothing currently needs attention.</p> : null}
    </div>
  );
}
