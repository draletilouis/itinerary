  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <a href="/api/reports/finance.csv" className="flex h-10 items-center gap-2 rounded-lg border bg-white px-4 text-sm font-semibold">
      ...
      <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          return <article key={String(label)} className="rounded-xl border bg-white p-5"><CardIcon className="size-5 text-[#011478]" /><p className="mt-4 text-xs text-[#6b7280]">{String(label)}</p><p className="mt-2 text-xl font-semibold">{String(value)}</p></article>;
        })}
      </section>
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Resource utilisation</h2><div className="mt-4 space-y-2">{data.resourceUtilisation.map((entry) => <div key={`${entry.resourceType}-${entry.status}`} className="flex items-center justify-between rounded-lg bg-[#f9fafb] p-3 text-sm"><span className="capitalize">{entry.resourceType.toLowerCase()} · {entry.status.toLowerCase()}</span><strong>{entry.count}</strong></div>)}</div></div>
        <div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Incident summary</h2><div className="mt-4 space-y-2">{data.incidentSummary.map((entry) => <div key={`${entry.severity}-${entry.status}`} className="flex items-center justify-between rounded-lg bg-[#f9fafb] p-3 text-sm"><span className="capitalize">{entry.severity.toLowerCase()} · {entry.status.toLowerCase()}</span><strong>{entry.count}</strong></div>)}</div></div>
      </section>
