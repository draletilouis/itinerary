    ...
    <div className="mx-auto max-w-[1600px]">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/customers" className="flex h-11 items-center rounded-xl border bg-white px-4 text-sm font-semibold">Customers</Link>
          <Link href="/enquiries/new" className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white">
      ...
      <div className="mt-7 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex w-fit rounded-xl border bg-white p-1">
          <Link
            ...
            className={`flex h-9 items-center gap-2 rounded-lg px-3 text-sm font-medium ${
              view !== "list" ? "bg-[#edf5f1] text-[#176b55]" : "text-[#68736e]"
            }`}
          >
        ...
        {view === "list" ? (
          <form className="flex flex-wrap gap-3">
            ...
            <span className="flex h-11 min-w-72 items-center gap-2 rounded-xl border bg-white px-3">
            ...
            <button className="h-11 rounded-xl border bg-white px-4 text-sm font-semibold">Apply</button>
          </form>
        ) : null}
      </div>
      ...
      {view === "list" ? (
        ...
        <section className="mt-5 grid gap-4 overflow-x-auto pb-3 xl:grid-cols-6">
          {columns.map((column) => {
            ...
            return (
              <div key={column.status} className="min-w-[270px] rounded-2xl bg-[#ebece7] p-3 xl:min-w-0">
                ...
                <div className="mt-2 space-y-3">
                  {items.map((enquiry) => (
                    <Link
                      ...
                      className="block rounded-xl border bg-white p-4 shadow-sm hover:border-[#b8cfc7]"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[11px] font-semibold text-[#176b55]">{enquiry.reference}</span>
                      ...
                      <p className="mt-2 line-clamp-2 text-xs leading-5 text-[#68736e]">
                        {enquiry.destinationsOfInterest.join(", ") || "Destination not set"}
                      </p>
                      <div className="mt-4 flex items-center gap-2 text-[11px] text-[#7b8580]">
                  ...
                  {!items.length ? (
                    <p className="rounded-xl border border-dashed border-[#ced2cb] px-3 py-7 text-center text-xs text-[#8b948f]">
                      No enquiries
                    </p>
                  ) : null}
                </div>
