...
export default async function DocumentsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#176b55]">PostgreSQL document control</p>
      <h1 className="mt-2 text-3xl font-semibold tracking-tight">Documents</h1>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-[#68736e]">Secure record-linked documents stored directly in PostgreSQL with authenticated downloads, size/type controls, optional expiry, and audit evidence.</p>

      <section className="mt-7 rounded-2xl border bg-white">
        <div className="flex items-center gap-3 border-b px-5 py-4"><FileUp className="size-5 text-[#176b55]" /><h2 className="font-semibold">Upload document</h2></div>
        <form action={uploadDocumentAction} className="grid gap-4 p-5 sm:grid-cols-2">
          ...
          <label className="text-xs sm:col-span-2">File<input className="mt-2 block w-full rounded-xl border bg-[#f8f8f5] p-3 text-sm" name="file" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.webp" required /></label>
          <label className="text-xs sm:col-span-2">Notes<input className={input} name="notes" /></label>
          <button className="h-10 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white sm:col-span-2">Upload securely</button>
        </form>
      </section>

      <section className="mt-6 overflow-hidden rounded-2xl border bg-white">
        <div className="flex items-center gap-3 border-b px-5 py-4"><FileCheck2 className="size-5 text-[#176b55]" /><div><h2 className="font-semibold">Document register</h2><p className="text-xs text-[#7b8580]">{attachments.length} latest records</p></div></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-[#f8f8f5] text-xs uppercase text-[#7b8580]"><tr><th className="px-5 py-3">Document</th><th className="px-5 py-3">Linked record</th><th className="px-5 py-3">Size</th><th className="px-5 py-3">Expiry</th><th className="px-5 py-3">Uploaded</th></tr></thead><tbody className="divide-y">{attachments.map((attachment) => <tr key={attachment.id}><td className="px-5 py-4"><a href={`/api/attachments/${attachment.id}`} className="font-semibold text-[#176b55]">{attachment.fileName}</a><p className="text-xs text-[#7b8580]">{attachment.documentType}</p></td><td className="px-5 py-4">{attachment.recordType} · <span className="font-mono text-xs">{attachment.recordId}</span></td><td className="px-5 py-4">{(attachment.fileSize / 1024).toFixed(1)} KB</td><td className="px-5 py-4">{attachment.expiresAt?.toLocaleDateString("en-UG") ?? "No expiry"}</td><td className="px-5 py-4">{attachment.createdAt.toLocaleString("en-UG")}<p className="text-xs text-[#7b8580]">{attachment.uploadedBy.fullName}</p></td></tr>)}{!attachments.length ? <tr><td colSpan={5} className="px-5 py-10 text-center text-[#7b8580]">No documents uploaded.</td></tr> : null}</tbody></table></div>
      </section>
      <p className="mt-6 flex items-center gap-2 text-xs text-[#7b8580]"><ShieldCheck className="size-4 text-[#176b55]" />There is no destructive delete workflow; corrections create new evidence.</p>
    </div>
  );
}
