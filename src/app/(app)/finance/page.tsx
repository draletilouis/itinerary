...
export default async function FinancePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <Link href="/reports" className="mt-5 inline-flex h-10 items-center rounded-xl border bg-white px-4 text-sm font-semibold">Open reports</Link>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        <details className="rounded-2xl border bg-white">
          ...
          <form action={recordCustomerPaymentAction} className="grid gap-4 border-t p-5 sm:grid-cols-2">
            ...
            <button className="h-10 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white sm:col-span-2">Record payment</button>
          </form>
        </details>
        ...
        <details className="rounded-2xl border bg-white">
          ...
          <form action={createAdditionalInvoiceAction} className="grid gap-4 border-t p-5 sm:grid-cols-2">
            ...
            <label className="text-xs">Notes<input className={input} name="notes" /></label>
            <button className="h-10 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white sm:col-span-2">Issue invoice</button>
          </form>
        </details>
        ...
        <details className="rounded-2xl border bg-white">
          ...
          <form action={createSupplierBillAction} className="grid gap-4 border-t p-5 sm:grid-cols-2">
            ...
            <label className="text-xs">Notes<input className={input} name="notes" /></label>
            <button className="h-10 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white sm:col-span-2">Record supplier bill</button>
          </form>
        </details>
        ...
        <details className="rounded-2xl border bg-white">
          ...
          <form action={recordTourExpenseAction} className="grid gap-4 border-t p-5 sm:grid-cols-2">
            ...
            <button className="h-10 rounded-xl bg-[#176b55] px-4 text-sm font-semibold text-white sm:col-span-2">Record expense</button>
          </form>
        </details>
      </section>
      ...
      <section className="mt-6 space-y-4">
        ...
        {data.payments.map((payment) => {
          ...
          return <article key={payment.id} className="rounded-2xl border bg-white p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold">{payment.receiptReference} · {payment.customer.fullName}</p><p className="mt-1 text-xs text-[#7b8580]">{formatMoney(payment.originalAmount.toString(),payment.paymentCurrencyCode)} at {payment.exchangeRate.toString()} = {formatMoney(payment.bookingCurrencyAmount.toString(),payment.bookingCurrencyCode)} · {payment.method}</p></div><div className="flex gap-2"><a href={`/api/payments/${payment.id}/receipt`} className="rounded-xl border px-3 py-2 text-xs font-semibold">Receipt PDF</a><span className="rounded-full bg-[#edf5f1] px-2.5 py-1 text-xs capitalize">{payment.status.toLowerCase()}</span></div></div>
            <div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-xl bg-[#f8f8f5] p-4 text-sm"><p className="text-xs text-[#7b8580]">Allocations</p>{payment.allocations.map((allocation) => <div key={allocation.id} className="mt-2 flex justify-between"><span>{allocation.invoice.reference}</span><span className="font-semibold">{formatMoney(allocation.invoiceCurrencyAmount.toString(),payment.bookingCurrencyCode)}</span></div>)}<p className="mt-3 border-t pt-3 text-xs">Unallocated: {formatMoney(Prisma.Decimal.max(available, 0).toString(),payment.bookingCurrencyCode)}</p></div>
              {payment.status === "RECORDED" ? <div className="space-y-3"><form action={allocateCustomerPaymentAction} className="grid gap-3 sm:grid-cols-[1fr_140px_auto]"><input type="hidden" name="paymentId" value={payment.id} /><select className="h-10 rounded-xl border px-3 text-xs" name="invoiceId" required defaultValue=""><option value="" disabled>Select invoice</option>{payment.booking.invoices.map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.reference} · {invoice.currencyCode} {invoice.balanceDue.toString()}</option>)}</select><input className="h-10 rounded-xl border px-3 text-xs" name="invoiceCurrencyAmount" required placeholder="Amount" /><button className="h-10 rounded-xl bg-[#176b55] px-3 text-xs font-semibold text-white">Allocate</button></form><form action={reverseCustomerPaymentAction} className="flex gap-2"><input type="hidden" name="paymentId" value={payment.id} /><input className="h-10 flex-1 rounded-xl border px-3 text-xs" name="reason" required placeholder="Reversal reason" /><button className="flex h-10 items-center gap-1 rounded-xl border px-3 text-xs text-red-700"><Undo2 className="size-3" /> Reverse</button></form></div> : null}
            </div>
            {payment.allocations.length ? <details className="mt-4"><summary className="cursor-pointer text-xs font-semibold text-[#176b55]">Record refund</summary><form action={recordRefundAction} className="mt-3 grid gap-3 sm:grid-cols-4"><select className="h-10 rounded-xl border px-3 text-xs" name="allocationId" required defaultValue=""><option value="" disabled>Select allocation</option>{payment.allocations.map((allocation) => <option key={allocation.id} value={allocation.id}>{allocation.invoice.reference} · {allocation.invoiceCurrencyAmount.toString()}</option>)}</select><input className="h-10 rounded-xl border px-3 text-xs" name="refundDate" type="date" required defaultValue={today} /><input className="h-10 rounded-xl border px-3 text-xs" name="bookingCurrencyAmount" required placeholder="Refund amount" /><input className="h-10 rounded-xl border px-3 text-xs" name="reason" required placeholder="Reason" /><button className="h-10 rounded-xl border px-3 text-xs font-semibold sm:col-span-4">Record refund</button></form></details> : null}
          </article>;
        })}
      </section>

      <section className="mt-6 grid gap-6 xl:grid-cols-2">
        <div className="space-y-3"><h2 className="font-semibold">Supplier bills</h2>{data.supplierBills.map((bill) => <article key={bill.id} className="rounded-2xl border bg-white p-5"><div className="flex justify-between gap-4"><div><p className="font-semibold">{bill.reference} · {bill.supplier.name}</p><p className="mt-1 text-xs text-[#7b8580]">{bill.tour?.name ?? "General"} · due {bill.dueDate.toLocaleDateString("en-UG")}</p></div><div className="text-right"><p className="font-semibold">{formatMoney(bill.balanceDue.toString(),bill.currencyCode)}</p><p className="text-xs capitalize text-[#7b8580]">{bill.status.toLowerCase().replaceAll("_"," ")}</p></div></div>{!["PAID","CANCELLED"].includes(bill.status) ? <details className="mt-4"><summary className="cursor-pointer text-xs font-semibold text-[#176b55]">Record supplier payment</summary><form action={recordSupplierPaymentAction} className="mt-3 grid gap-3 sm:grid-cols-2"><input type="hidden" name="billId" value={bill.id} /><input className="h-10 rounded-xl border px-3 text-xs" name="paymentDate" type="date" required defaultValue={today} /><select className="h-10 rounded-xl border px-3 text-xs" name="paymentCurrencyCode" required defaultValue={bill.currencyCode}>{data.currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.code}</option>)}</select><input className="h-10 rounded-xl border px-3 text-xs" name="originalAmount" required placeholder="Amount paid" /><input className="h-10 rounded-xl border px-3 text-xs" name="exchangeRate" required defaultValue="1" placeholder="Rate to bill currency" /><input className="h-10 rounded-xl border px-3 text-xs" name="exchangeRateDate" type="date" required defaultValue={today} /><input className="h-10 rounded-xl border px-3 text-xs" name="method" required placeholder="Method" /><input className="h-10 rounded-xl border px-3 text-xs" name="externalReference" placeholder="Reference" /><input className="h-10 rounded-xl border px-3 text-xs" name="notes" placeholder="Notes" /><button className="h-10 rounded-xl bg-[#176b55] px-3 text-xs font-semibold text-white sm:col-span-2">Record payment</button></form></details> : null}</article>)}</div>
        <div className="space-y-3"><h2 className="font-semibold">Tour expenses</h2>{data.expenses.map((expense) => <article key={expense.id} className="rounded-2xl border bg-white p-5"><div className="flex justify-between gap-4"><div><p className="font-semibold">{expense.reference} · {expense.description}</p><p className="mt-1 text-xs text-[#7b8580]">{expense.tour.reference} · {expense.category} · {expense.supplier?.name ?? "No supplier"}</p></div><div className="text-right"><p className="font-semibold">{formatMoney(expense.convertedAmount.toString(),expense.costingCurrencyCode)}</p><p className="text-xs text-[#7b8580]">{formatMoney(expense.originalAmount.toString(),expense.originalCurrencyCode)} at {expense.exchangeRate.toString()}</p></div></div>{expense.status === "RECORDED" ? <form action={reverseTourExpenseAction} className="mt-4 flex gap-2"><input type="hidden" name="expenseId" value={expense.id} /><input className="h-10 flex-1 rounded-xl border px-3 text-xs" name="reason" required placeholder="Reversal reason" /><button className="flex h-10 items-center gap-1 rounded-xl border px-3 text-xs text-red-700"><Undo2 className="size-3" /> Reverse</button></form> : <p className="mt-3 text-xs text-red-700">Reversed</p>}</article>)}</div>
      </section>
