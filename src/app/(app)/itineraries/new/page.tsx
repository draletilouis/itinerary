...
export default async function NewItineraryPage({
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-2 text-sm text-[#4b5563]">The first draft automatically receives one day for every tour day.</p>
      <form action={createItineraryAction} className="mt-7 space-y-5 rounded-xl border bg-white p-6">
        ...
        <label className="block text-sm font-medium">Introduction<textarea className="mt-2 min-h-32 w-full rounded-lg border p-3 text-sm" name="introduction" /></label>
        <div className="flex justify-end"><button className="h-11 rounded-lg bg-[#011478] px-5 text-sm font-semibold text-white">Create itinerary</button></div>
      </form>
    </div>
  );
}
