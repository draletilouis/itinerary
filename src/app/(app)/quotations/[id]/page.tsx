      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="space-y-6">
          ...
          {canRevise && version.versionNumber === quotation.currentVersionNumber ? <section className="rounded-2xl border bg-white p-5"><h2 className="font-semibold">Create revision</h2><p className="mt-1 text-xs text-[#7b8580]">The current version becomes superseded; its snapshots remain unchanged.</p><form action={reviseQuotationAction} className="mt-5 grid gap-4 sm:grid-cols-2"><input type="hidden" name="quotationId" value={quotation.id} />
            ...
            <label className="text-xs sm:col-span-2">Quotation view<select className={input} name="presentationMode" defaultValue={version.presentationMode}><option value="BOTH">Itinerary items + per traveller</option><option value="ITEMIZED">Itinerary items only</option><option value="PER_TRAVELLER">Per traveller only</option></select></label><div className="rounded-xl border bg-[#fafaf7] p-3 text-xs sm:col-span-2"><p className="font-semibold">Optional custom traveller prices</p><p className="mt-1 leading-5 text-[#7b8580]">Leave blank to balance the revised total automatically.</p><div className="mt-2 grid gap-3 sm:grid-cols-2">{quotation.tour.adults > 0 ? <input className={input} name="adultUnitPrice" inputMode="decimal" placeholder={`Adult price (current ${version.adultUnitPrice?.toFixed(2) ?? "auto"})`} /> : null}{quotation.tour.children > 0 ? <input className={input} name="childUnitPrice" inputMode="decimal" placeholder={`Child price (current ${version.childUnitPrice?.toFixed(2) ?? "auto"})`} /> : null}</div></div>
