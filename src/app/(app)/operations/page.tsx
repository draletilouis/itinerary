...
export default async function OperationsPage({ searchParams }: { searchParams: Promise<{ tour?: string; prepared?: string; tasks?: string; suppliers?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      {query.prepared === "1" ? (
        <section className="mt-7 rounded-xl border border-[#bfdbfe] bg-[#eff3ff] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#011478]">Booking confirmed - preparation created</p>
          <h2 className="mt-2 text-lg font-semibold">Continue with the highlighted tour below</h2>
          <p className="mt-1 text-sm text-[#4b5563]">Created automatically: {query.tasks ?? "0"} preparation task(s). {query.suppliers ?? "0"} supplier confirmation record(s) are available for optional tracking.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg bg-white p-3"><p className="text-xs font-semibold">1. Complete travellers</p><p className="mt-1 text-[11px] text-[#4b5563]">Add the lead guest and missing traveller details.</p></div>
            <div className="rounded-lg bg-white p-3"><p className="text-xs font-semibold">2. Assign resources</p><p className="mt-1 text-[11px] text-[#4b5563]">Select the required vehicle, driver and guide.</p></div>
            <div className="rounded-lg bg-white p-3"><p className="text-xs font-semibold">3. Generate and start</p><p className="mt-1 text-[11px] text-[#4b5563]">Generate the operations pack, clear blockers and start.</p></div>
          </div>
        </section>
      ) : null}
      ...
      <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          return <article key={String(label)} className="rounded-xl border bg-white p-5"><CardIcon className="size-5 text-[#011478]" /><p className="mt-4 text-xs text-[#6b7280]">{String(label)}</p><p className="mt-2 text-2xl font-semibold">{String(value)}</p></article>;
        })}
      </section>

      <section className="mt-6 space-y-5">
        {data.tours.map((tour) => (
          <article id={`tour-${tour.id}`} key={tour.id} className={`scroll-mt-6 overflow-hidden rounded-xl border bg-white ${selectedTourId === tour.id ? "ring-2 ring-[#011478] ring-offset-2" : ""}`}>
            <div className="border-b bg-[#111827] px-6 py-5 text-white">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-[#67bfff]">{tour.reference}  -  {tour.status.toLowerCase().replaceAll("_"," ")}</p>
                ...
                <div className="min-w-56 rounded-lg bg-white/10 p-4">
                  ...
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-[#67bfff]" style={{ width: `${tour.readiness.score}%` }} /></div>
              ...
              <div className="space-y-6">
                <section>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {tour.status === "IN_PROGRESS" ? <span className="rounded-lg bg-[#f3f4f6] px-3 py-2 text-xs font-semibold text-[#4b5563]">Readiness locked - tour started</span> : <form action={refreshTourReadinessAction}><input type="hidden" name="tourId" value={tour.id} /><button className="rounded-lg border px-3 py-2 text-xs font-semibold">Recalculate status</button></form>}
                  </div>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {tour.readiness.checks.map((check) => {
                      ...
                      const card = <div className={`rounded-lg border p-3 ${check.passed ? "bg-[#eff3ff]" : "bg-amber-50"}`}><div className="flex gap-2"><CheckCircle2 className={`mt-0.5 size-4 shrink-0 ${check.passed ? "text-[#011478]" : "text-amber-700"}`} /><div><p className="text-xs font-semibold">{check.label}</p><p className="mt-1 text-[11px] leading-4 text-[#6b7280]">{check.detail}</p>{href ? <p className="mt-2 text-[11px] font-semibold text-[#011478]">Click to fix</p> : null}</div></div></div>;
                      return href ? <Link key={check.key} href={href} className="block rounded-lg outline-none transition hover:-translate-y-0.5 hover:shadow-sm focus:ring-2 focus:ring-[#011478]">{card}</Link> : <div key={check.key}>{card}</div>;
                    })}
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {!tour.operationalTasks.length ? <form action={initializeTourOperationsAction}><input type="hidden" name="tourId" value={tour.id} /><button className="rounded-lg bg-[#011478] px-4 py-2 text-xs font-semibold text-white">Initialize operations checklist</button></form> : null}
                    {tour.readiness.ready && tour.status !== "IN_PROGRESS" && tour.status !== "COMPLETED" ? <form action={setTourStatusAction}><input type="hidden" name="tourId" value={tour.id} /><input type="hidden" name="status" value="IN_PROGRESS" /><input type="hidden" name="reason" value="Operations started after readiness controls passed." /><button className="rounded-lg bg-[#111827] px-4 py-2 text-xs font-semibold text-white">Start tour</button></form> : null}
                    {tour.status === "IN_PROGRESS" ? <form action={setTourStatusAction}><input type="hidden" name="tourId" value={tour.id} /><input type="hidden" name="status" value="COMPLETED" /><input type="hidden" name="reason" value="Tour operations completed." /><button className="rounded-lg border px-4 py-2 text-xs font-semibold">Complete tour</button></form> : null}
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {tour.resourceAssignments.map((entry) => <div key={entry.id} className="rounded-lg bg-[#f9fafb] p-3"><p className="text-xs font-semibold">{entry.resourceType}  -  {resourceName(entry)}</p><p className="mt-1 text-[11px] text-[#6b7280]">{entry.startDate.toLocaleDateString("en-UG")}  -  {entry.endDate.toLocaleDateString("en-UG")}  -  {entry.status.toLowerCase()}</p></div>)}
                    {!tour.resourceAssignments.length ? <p className="text-sm text-[#6b7280]">No resources assigned.</p> : null}
                  </div>
                </section>
                ...
                <section>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[#6b7280]">{tour.operationalTasks.filter((task) => !["COMPLETED","WAIVED"].includes(task.status)).length} remaining</span>
                  </div>
                  <div className="mt-3 space-y-2">
                    {tour.operationalTasks.filter((task) => !["COMPLETED","WAIVED"].includes(task.status)).map((task) => <div key={task.id} className="rounded-lg border p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">{task.title}{task.mandatory ? <span className="ml-2 text-[10px] uppercase text-red-700">Required</span> : <span className="ml-2 text-[10px] uppercase text-[#4b5563]">Optional</span>}</p><p className="mt-1 text-xs text-[#6b7280]">{task.dueDate ? `Due ${task.dueDate.toLocaleDateString("en-UG")}` : "No due date"}</p></div><form action={setOperationalTaskStatusAction} className="flex gap-2"><input type="hidden" name="taskId" value={task.id} /><input className="h-8 rounded-lg border px-2 text-xs" name="reason" placeholder="Reason if waived" /><button name="status" value="COMPLETED" className="rounded-lg border px-2 text-xs">Done</button><button name="status" value="WAIVED" className="rounded-lg border px-2 text-xs text-amber-700">Skip</button></form></div></div>)}
                    {!tour.operationalTasks.some((task) => !["COMPLETED","WAIVED"].includes(task.status)) ? <p className="rounded-lg bg-[#eff3ff] p-3 text-sm text-[#011478]">All preparation tasks are complete.</p> : null}
                  </div>
                  {tour.operationalTasks.some((task) => ["COMPLETED","WAIVED"].includes(task.status)) ? <details className="mt-3 rounded-lg border"><summary className="cursor-pointer px-4 py-3 text-xs font-semibold text-[#4b5563]">Completed tasks ({tour.operationalTasks.filter((task) => ["COMPLETED","WAIVED"].includes(task.status)).length})</summary><div className="space-y-2 border-t p-3">{tour.operationalTasks.filter((task) => ["COMPLETED","WAIVED"].includes(task.status)).map((task) => <div key={task.id} className="flex justify-between gap-3 text-xs"><span>{task.title}</span><span className="capitalize text-[#6b7280]">{task.status.toLowerCase()}</span></div>)}</div></details> : null}
                  <details className="mt-3"><summary className="cursor-pointer text-xs font-semibold text-[#011478]">Add another task</summary><form action={createOperationalTaskAction} className="mt-3 grid gap-3 rounded-lg bg-[#f9fafb] p-4 sm:grid-cols-2"><input type="hidden" name="tourId" value={tour.id} /><label className="text-xs">Title<input className={input} name="title" required /></label><label className="text-xs">Due date<input className={input} name="dueDate" type="date" /></label><label className="text-xs sm:col-span-2">Description<input className={input} name="description" /></label><label className="flex items-center gap-2 text-xs"><input name="mandatory" type="checkbox" /> Required before starting</label><button className="h-10 rounded-lg border px-3 text-xs font-semibold sm:col-span-2">Add task</button></form></details>
                </section>
              </div>

              <div className="space-y-6">
                <details className="rounded-lg border p-4">
                  <summary className="cursor-pointer text-sm font-semibold">Supplier confirmations <span className="font-normal text-[#4b5563]">(optional - {tour.supplierConfirmations.length})</span></summary>
                  <div className="mt-3 space-y-2">
                    {tour.supplierConfirmations.map((confirmation) => <div key={confirmation.id} className="rounded-lg border p-3"><div className="flex justify-between gap-3"><div><p className="text-sm font-semibold">{confirmation.supplier.name}</p><p className="mt-1 text-xs text-[#6b7280]">{confirmation.service}  -  {confirmation.serviceDate?.toLocaleDateString("en-UG") ?? "Date not set"}</p></div><span className="text-xs capitalize">{confirmation.status.toLowerCase()}</span></div>{confirmation.status !== "CONFIRMED" && confirmation.status !== "CANCELLED" ? <form action={setSupplierConfirmationStatusAction} className="mt-3 grid gap-2 sm:grid-cols-2"><input type="hidden" name="confirmationId" value={confirmation.id} /><input className="h-8 rounded-lg border px-2 text-xs" name="confirmedByName" placeholder="Confirmed by" /><input className="h-8 rounded-lg border px-2 text-xs" name="externalReference" placeholder="Reference" /><input className="h-8 rounded-lg border px-2 text-xs sm:col-span-2" name="notes" placeholder="Notes" /><div className="flex gap-2 sm:col-span-2"><button name="status" value="REQUESTED" className="rounded-lg border px-2 py-1 text-xs">Mark requested</button><button name="status" value="CONFIRMED" className="rounded-lg bg-[#011478] px-2 py-1 text-xs text-white">Confirm</button><button name="status" value="DECLINED" className="rounded-lg border px-2 py-1 text-xs text-red-700">Declined</button></div></form> : null}</div>)}
                  </div>
                  <details className="mt-3"><summary className="cursor-pointer text-xs font-semibold text-[#011478]">Add supplier service</summary><form action={createSupplierConfirmationAction} className="mt-3 grid gap-3 rounded-lg bg-[#f9fafb] p-4 sm:grid-cols-2"><input type="hidden" name="tourId" value={tour.id} /><label className="text-xs sm:col-span-2">Supplier<select className={input} name="supplierId" required defaultValue=""><option value="" disabled>Select supplier</option>{data.suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name} / {supplier.category.name}</option>)}</select></label><label className="text-xs">Service<input className={input} name="service" required /></label><label className="text-xs">Service date<input className={input} name="serviceDate" type="date" /></label><label className="text-xs">External reference<input className={input} name="externalReference" /></label><label className="text-xs">Notes<input className={input} name="notes" /></label><button className="h-10 rounded-lg border px-3 text-xs font-semibold sm:col-span-2">Add confirmation</button></form></details>
                </details>

                <section>
                  <div className="flex items-center justify-between"><h3 className="font-semibold">Incidents</h3><span className="text-xs text-[#6b7280]">{tour.incidents.length}</span></div>
                  <div className="mt-3 space-y-2">
        ...
        {!data.tours.length ? <div className="rounded-xl border bg-white p-10 text-center"><UsersRound className="mx-auto size-8 text-[#011478]" /><p className="mt-4 font-semibold">No confirmed tours need operations yet.</p><p className="mt-2 text-sm text-[#6b7280]">Accept a quotation and confirm its booking first.</p></div> : null}
      </section>
    </div>
  );
}
