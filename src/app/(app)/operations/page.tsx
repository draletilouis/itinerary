...
function resourceName(entry: {
  ...
  if (entry.vehicle) return `${entry.vehicle.registration} Â· ${entry.vehicle.make} ${entry.vehicle.model}`;
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#176b55]">
        Phase 10 Â· Operational control
      </p>
          ...
          <article key={tour.id} className="overflow-hidden rounded-2xl border bg-white">
            <div className="border-b bg-[#123d32] px-6 py-5 text-white">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-[#eac58f]">{tour.reference} Â· {tour.status.toLowerCase().replaceAll("_"," ")}</p>
                  <h2 className="mt-2 text-xl font-semibold">{tour.name}</h2>
                  <p className="mt-1 text-sm text-white/65">{tour.customer.fullName} Â· {tour.startDate.toLocaleDateString("en-UG")} â€“ {tour.endDate.toLocaleDateString("en-UG")}</p>
                </div>
            ...
            <div className="grid gap-6 p-6 xl:grid-cols-[1.2fr_1fr]">
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {tour.resourceAssignments.map((entry) => <div key={entry.id} className="rounded-xl bg-[#f8f8f5] p-3"><p className="text-xs font-semibold">{entry.resourceType} Â· {resourceName(entry)}</p><p className="mt-1 text-[11px] text-[#7b8580]">{entry.startDate.toLocaleDateString("en-UG")} â€“ {entry.endDate.toLocaleDateString("en-UG")} Â· {entry.status.toLowerCase()}</p></div>)}
              ...
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <div className="mt-3 space-y-2">{tour.operationalDocuments.map((document) => <a key={document.id} href={`/api/operations/documents/${document.id}/pdf`} className="flex items-center justify-between rounded-xl border p-3 text-xs"><span><strong>{document.reference}</strong> Â· {document.title}<span className="ml-2 text-[#7b8580]">{document.createdAt.toLocaleString("en-UG")}</span></span><FileDown className="size-4 text-[#176b55]" /></a>)}</div>
                </section>
              </div>
            </div>
          </article>
