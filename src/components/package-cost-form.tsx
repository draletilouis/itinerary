...
export function PackageCostForm({ packageId, durationDays, defaultTravellers, costingCurrencyCode, currencies, suppliers, itineraryDays }: {
  ...
  return (
    <form action={addPackageCostAction} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {basis !== "OVERRIDE" ? <label className="text-xs font-medium">Unit rate<input className={fieldClass} name="unitCost" type="number" min={0} step="0.01" value={unitCost} onChange={(event) => setUnitCost(event.target.value)} required /></label> : <input type="hidden" name="unitCost" value="0" />}
      ...
      {basis.startsWith("PER_PERSON") ? <label className="text-xs font-medium">People charged<input className={fieldClass} name="eligibleTravellers" type="number" min="1" value={travellers} onChange={(event) => setTravellers(event.target.value)} /></label> : null}
