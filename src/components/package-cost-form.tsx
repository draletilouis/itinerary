...
type Rate = { id: string; service: string; unit: string; amount: string | { toString(): string }; currencyCode: string; startDate: Date | string; endDate: Date | string | null };
...
export function PackageCostForm({ packageId, durationDays, defaultTravellers, costingCurrencyCode, currencies, suppliers, itineraryDays }: {
  ...
  return (
    <form action={addPackageCostAction} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {basis.startsWith("PER_PERSON") ? <label className="flex items-center gap-2 rounded-lg border p-3 text-xs font-medium sm:col-span-2"><input type="checkbox" name="useTravellerRateBands" value="true" checked={useTravellerRateBands} onChange={(event) => setUseTravellerRateBands(event.target.checked)} />Use Ugandan, East African and non-East African rates</label> : null}
      {useTravellerRateBands && basis.startsWith("PER_PERSON") ? <div className="grid gap-3 rounded-lg border bg-[#f9fafb] p-4 sm:col-span-2 lg:col-span-4 sm:grid-cols-2 lg:grid-cols-3"><p className="text-xs text-gray-600 sm:col-span-2 lg:col-span-3">Enter each applicable rate. Leave an amount blank when that band is not offered.</p>{[["UGANDAN", "ADULT", "Ugandan adult"], ["UGANDAN", "CHILD", "Ugandan child"], ["EAST_AFRICAN", "ADULT", "East African adult"], ["EAST_AFRICAN", "CHILD", "East African child"], ["NON_EAST_AFRICAN", "ADULT", "Non-East African adult"], ["NON_EAST_AFRICAN", "CHILD", "Non-East African child"]].map(([category, age, label]) => <div key={category + "-" + age} className="rounded-lg border bg-white p-3"><p className="text-xs font-semibold">{label}</p><div className="mt-2 grid grid-cols-[1fr_90px] gap-2"><input className={fieldClass} name={"rate_" + category + "_" + age} type="number" min="0" step="0.01" placeholder="Rate" /><select className={fieldClass} name={"currency_" + category + "_" + age} defaultValue={currency}>{currencies.map((entry) => <option key={entry.code}>{entry.code}</option>)}</select></div></div>)}</div> : null}
